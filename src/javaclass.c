...
int constant_pool_add_Fieldref(struct ClassFile *cf, const char *class_name, const char *field_name,
...
int constant_pool_add_Methodref(struct ClassFile *cf, const char *class_name, const char *method_name,
                                const char *descriptor) {
    struct Pair t;
    struct ConstantPool **p = &cf->constant_pool;
    int index = 1;
...
static int save_u4(FILE *fd, unsigned int n) {
    fprintf(fd, "%c%c%c%c", (int) ((n >> 24) & 0xFF), (int) ((n >> 16) & 0xFF), (int) ((n >> 8) & 0xFF),
            (int) (n & 0xFF));
